...
# sam-3d-objects/notebook/inference.py imports omegaconf AND hydra directly. Both are
# normally pulled in transitively via hydra-core through '.[dev]' above, but a repair
# session that re-registers sam3d_objects via '-e . --no-deps' skips them entirely and the
